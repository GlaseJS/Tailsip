...
class SocketRoom {
    ...
    join(who, emit = Config.socket.roomAutoEmit) {
        if (who in this.sockets)
            return;
        this.sockets.push(who);
        if (emit)
            this.emit(who, "room:join");
    }
    leave(who, emit = Config.socket.roomAutoEmit) {
        if (!(who in this.sockets))
            return;
        this.sockets.splice(this.sockets.indexOf(who), 1);
        if (emit)
            this.emit(who, "room:leave");
    }
}
export const rooms = {
    "/": new SocketRoom()
